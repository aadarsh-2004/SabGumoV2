...
app.post('/api/send-email', async (req, res) => {
  ...
  try {
    const insertData = {
      ...
      destination,
      guests,
      traveldates: travelDates, // Use lowercase as per actual schema
      message,
      received_at: new Date().toISOString(),
      is_completed: false
    };
    
    console.log('Inserting inquiry into Supabase:', JSON.stringify(insertData, null, 2));
...
app.post('/api/inquiries', async (req, res) => {
  ...
  try {
    ...
    console.log('Inserting inquiry into Supabase via /api/inquiries:', JSON.stringify(insertData, null, 2));
    ...
    res.status(500).json({ message: 'Error submitting inquiry', details: err.message });
  }
});
